...
/**
 * A {@code LocationCallback} is used to run code after a Location has been fetched by {@link
 * ParseGeoPoint#getCurrentLocationInBackground(long, android.location.Criteria)}.
 *
 * <p>The easiest way to use a {@code LocationCallback} is through an anonymous inner class.
 * Override the {@code done} function to specify what the callback should do after the location has
 * been fetched. The {@code done} function will be run in the UI thread, while the location check
 * happens in a background thread. This ensures that the UI does not freeze while the fetch happens.
 *
 * <p>For example, this sample code defines a timeout for fetching the user's current location, and
