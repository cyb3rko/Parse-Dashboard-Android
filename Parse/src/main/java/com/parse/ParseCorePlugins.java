...
class ParseCorePlugins {
    ...
    private final AtomicReference<ParseObjectSubclassingController> subclassingController =
            new AtomicReference<>();
    private final AtomicReference<ParseSchemaController> schemaController = new AtomicReference<>();
    ...
    public ParseSchemaController getSchemaController() {
        if (schemaController.get() == null){
            ParseSchemaController controller = new ParseSchemaController(ParsePlugins.get().restClient());
            schemaController.compareAndSet(null, controller);
        }
        return schemaController.get();
    }
